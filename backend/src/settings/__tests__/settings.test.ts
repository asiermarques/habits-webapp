...
describe('Settings API', () => {
  ...
  it('PUT /settings/currency updates the currency', async () => {
    const put = await request(app).put('/api/settings/currency').send({ currency: 'USD' });
  ...
  it('PUT /settings/currency rejects unsupported codes', async () => {
    const res = await request(app).put('/api/settings/currency').send({ currency: 'XYZ' });
    expect(res.status).toBe(400);
  });

  it('PUT /settings/currency rejects missing body', async () => {
    const res = await request(app).put('/api/settings/currency').send({});
    expect(res.status).toBe(400);
  });
  ...
  it('PUT /settings/locale updates the locale', async () => {
    const put = await request(app).put('/api/settings/locale').send({ locale: 'es' });
    expect(put.status).toBe(200);
  ...
  it('PUT /settings/locale rejects unsupported codes', async () => {
    const res = await request(app).put('/api/settings/locale').send({ locale: 'fr' });
    expect(res.status).toBe(400);
  });

  it('PUT /settings/locale rejects missing body', async () => {
    const res = await request(app).put('/api/settings/locale').send({});
    expect(res.status).toBe(400);
  });
});
