...
describe('Users API', () => {
  ...
  describe('POST /users', () => {
    ...
    it('rejects empty or missing names', async () => {
      ...
      const r2 = await request(app).post('/api/users').send({ name: '   ' });
      expect(r1.status).toBe(400);
      expect(r2.status).toBe(400);
    });

    it('rejects names longer than 100 characters', async () => {
      const res = await request(app).post('/api/users').send({ name: 'a'.repeat(101) });
  ...
  describe('DELETE /users/:id', () => {
    ...
    it('promotes another user to default if the deleted one was default', async () => {
      ...
      await request(app).delete(`/api/users/${alice.id}`);
      const list = await request(app).get('/api/users');
