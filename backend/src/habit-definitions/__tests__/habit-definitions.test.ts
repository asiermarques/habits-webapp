...
describe('Habit Definitions API', () => {
  describe('GET /habit-definitions', () => {
    it('rejects without userId', async () => {
      const res = await request(app).get('/api/habit-definitions');
    ...
    it('scopes habits to the requested user', async () => {
      ...
      const aliceList = (await request(app).get(`/api/habit-definitions?userId=${alice.id}`))
        .body as HabitDefinition[];
      const bobList = (await request(app).get(`/api/habit-definitions?userId=${bob.id}`))
        .body as HabitDefinition[];
  ...
  describe('PUT /habit-definitions/:id', () => {
    it('updates the name', async () => {
      ...
      const res = await request(app).put(`/api/habit-definitions/${habit.id}`).send({ name: 'Books' });
      expect(res.status).toBe(200);
    ...
    it('toggles positive on a custom habit (no entries yet, so type changes are allowed)', async () => {
      ...
      const res = await request(app).put(`/api/habit-definitions/${habit.id}`).send({ positive: false });
    ...
    it('forces positive back to true when changing type to workout/writing', async () => {
      ...
      const res = await request(app).put(`/api/habit-definitions/${habit.id}`).send({ type: 'workout' });
      expect(res.body.type).toBe('workout');
      expect(res.body.positive).toBe(true);
    });

    it('returns 404 for an unknown id', async () => {
      const res = await request(app).put('/api/habit-definitions/9999').send({ name: 'Ghost' });
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('DELETE /habit-definitions/:id', () => {
    it('deletes a habit definition', async () => {
      const user = await createUser('Alice');
      const habit = await createHabit(user.id, { name: 'Reading', type: 'custom' });
      const res = await request(app).delete(`/api/habit-definitions/${habit.id}`);
      expect(res.status).toBe(204);

      const list = await request(app).get(`/api/habit-definitions?userId=${user.id}`);
