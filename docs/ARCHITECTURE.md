...
- **Offline mutations** — scoped to **Entries** only; **Habit Definitions**, **Users** and **Settings** stay online-only. `apiFetch` (`frontend/src/lib/api.ts`) wraps `fetch` and, when the request never reaches the server (a network-level `TypeError`), throws a distinct `OfflineError` rather than a generic failure. The global `MutationCache.onError`/`QueryCache.onError` handler in `frontend/src/main.tsx` maps it to a clear "you're offline" Sonner toast for every mutation still covered by that non-goal. Those mutations carry no optimistic updates (they only invalidate `onSuccess`), so a failed offline write never appears saved.
  - **Entry writes** (`frontend/src/entries/offlineStore.ts`, `queries.ts`, `sync.ts`, `EntrySync.tsx`) are the one exception. `useCreateEntry` catches `OfflineError` and, instead of failing, writes the create to a durable `localStorage`-backed store (`offlineStore.ts`) keyed by a client-only negative `localId` that is never sent to the server. The store write is synchronous; if it throws (e.g. quota exceeded) the mutation fails for real rather than silently dropping the Entry (BR-001). Pending Entries are **overlaid at read time** onto `useEntriesInfinite` (`EntriesList`) and `useWeeklyMetrics` (`WeekChartSection`, via `metrics/pendingOverlay.ts`) rather than injected into TanStack Query's cache — cursor pagination on `(date DESC, id DESC)` would otherwise break. `EntrySync` (mounted once in `App.tsx`) drains the store sequentially against `POST /api/entries` on mount, on the browser's `online` event, and every 30s while online, using `apiFetch` directly (not a mutation) so a failing background drain never spams the global error toast; a Header pill (`usePendingChangesCount`) shows the total pending count, absent at zero.
  - **Known gap (GRISK-001, tracked by `002-entry-sync-protocol.md`)**: the push is at-least-once, not exactly-once. If `POST /entries` succeeds but the response is lost (e.g. the connection drops mid-response), the next drain retries and creates a duplicate Entry. This can't be deduped client-side — multiple Entries per `(user, definition, date)` are legal — so it's an accepted cost until `002` adds a real idempotency key.
