...
- **Offline mutations** — there are no offline writes (NOGOAL-001). `apiFetch` (`frontend/src/lib/api.ts`) wraps `fetch` and, when the request never reaches the server (a network-level `TypeError`), throws a distinct `OfflineError` rather than a generic failure. The global `MutationCache.onError`/`QueryCache.onError` handler in `frontend/src/main.tsx` maps it to a clear "you're offline" Sonner toast. Mutations carry no optimistic updates (they only invalidate `onSuccess`), so a failed offline write never appears saved.
