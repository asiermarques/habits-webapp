...
- **Sync resilience for offline changes** — the indicator distinguishes "pending" (waiting, including simply being offline) from "failing" (online, but repeated attempts to reach the server haven't gone through), and a failing state offers a manual retry. A single transient error never shows as failing — only a run of consecutive failures does, and it clears the moment a drain succeeds. A retried push is exactly-once: every pending change carries an opaque key generated when it's queued, and the backend recognizes a repeat of the same change instead of applying it twice, so a dropped response can never double-log an Entry. Not every server refusal is simply discarded, either: a queued deletion whose Entry was already removed elsewhere settles silently (the user's intent was already satisfied — no toast, no failure); a queued edit whose Entry was deleted elsewhere is recreated with the edited values under a new id and announced with a toast naming the habit and the date, so the edit isn't lost and the reappearance isn't a mystery. Only when recreation itself is impossible (its Habit Definition or the User is also gone) does the change fall back to being discarded automatically with a toast explaining why, rather than retried forever or left blocking the rest of the backlog. Users can also clear a permanently stuck backlog themselves via **Discard pending changes** in Settings (above)
