...
- **Installable PWA, offline Entry logging** — the app ships a web manifest and a service worker, so it installs to the home screen and opens offline. A dismissible in-app prompt offers installation on Android/Chrome and shows manual "Add to Home Screen" guidance on iOS Safari (it never nags — hidden once installed or dismissed). The app shell is precached and previously-viewed API reads (Home, Metrics) are served from cache when the network is down, preferring fresh data when online. A newly deployed build reaches installed users via an in-app "refresh to update" prompt. **Logging an Entry offline works** — it's written to a durable local store, appears immediately in the Home entries list and weekly chart indistinguishable from a synced Entry, and is pushed to the backend automatically once the app is back online (a single header indicator says how many changes are still pending). This is scoped to Entries only: creating/editing a **Habit Definition**, managing **Users**, and changing **Settings** still require a connection and fail with a clear "you're offline" message when attempted offline. When the optional instance gate is enabled, cached API data is served offline only within a 2-hour grace window of the last online unlock; past that window (or after logout, which also evicts the cache) the unlock screen is shown instead, so caching only ever weakens the gate for a bounded time after a confirmed unlock
