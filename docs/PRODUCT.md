...
- **Installable PWA, read-only offline** — the app ships a web manifest and a service worker, so it installs to the home screen and opens offline. The app shell is precached and previously-viewed API reads (Home, Metrics) are served from cache when the network is down, preferring fresh data when online. A newly deployed build reaches installed users via an in-app "refresh to update" prompt. **Offline writes are a permanent non-goal** — logging, editing, and deleting entries always require a connection. When the optional instance gate is enabled, cached API data is not served offline (the unlock screen is shown instead) and is evicted on logout, so caching never weakens the gate
